...
1. [Karpathy, "Let's build GPT: from scratch"](https://www.youtube.com/watch?v=kCc8FmEb1nY) + [nanoGPT](https://github.com/karpathy/nanoGPT) — build one; don't just watch. **Note:** this is the capstone of his [Zero to Hero series](https://karpathy.ai/zero-to-hero.html) and assumes the earlier micrograd/makemore videos. Don't do all ~15 hours: watch [micrograd](https://www.youtube.com/watch?v=VMj-3S1tku0) only if backprop-from-scratch feels rusty; [makemore part 1](https://www.youtube.com/watch?v=PaCmpygFfXo) + part 2 (MLP) are a ~4-hour on-ramp to his PyTorch idioms; parts 3–5 (BatchNorm, backprop ninja, WaveNet) are training-focused — skip for an inference-hardware goal. Fallback: start the GPT video directly and backfill by pausing.
